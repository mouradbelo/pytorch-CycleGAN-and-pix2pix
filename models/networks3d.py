...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='replicate'): # Use of replicate instead of reflect !MB as reflect not implemented !!!!!
        ...
        # Add 3D reflect padding here => forward method
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.ConvTranspose3d(ngf * mult, int(ngf * mult / 2),
                      ...
                      nn.ReLU(True)]
        # Add 3D reflect padding here
        model_2 = [nn.Conv3d(ngf, output_nc, kernel_size=7, padding=0)]
        model_2 += [nn.Tanh()]
...
#         import pudb;pudb.set_trace()
        # Since there's no 3d nn module for padding, break down and use functional.pad
        input = nn.functional.pad(input, (3,3,3,3,3,3), self.padding_type)
        input = self.model_1(input)
        input = nn.functional.pad(input, (3,3,3,3,3,3), self.padding_type)
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        self.padding_type = padding_type
        self.conv_block_1, self.conv_block_2 = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias)
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        p = 0
        # Padding here
        # Padding is now done in the forward function for 3D input
        # Only reflect and replicate are supported for now, zero padding isn't supported
        conv_block_1 += [nn.Conv3d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
            conv_block_1 += [nn.Dropout(0.5)]
        # Padding here
        conv_block_2 = [nn.Conv3d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]

        return nn.Sequential(*conv_block_1), nn.Sequential(*conv_block_2)

    def forward(self, x):
        """Forward function (with skip connections)"""
        if self.padding_type == 'reflect':
            out = nn.functional.pad(x, (1,1,1,1,1,1), 'reflect')
        elif self.padding_type == 'replicate':
            out = nn.functional.pad(x, (1,1,1,1,1,1), "replicate")
        else:
        ...
        if self.padding_type == 'reflect':
            out = nn.functional.pad(out, (1,1,1,1,1,1), 'reflect')
        elif self.padding_type == 'replicate':
            out = nn.functional.pad(out, (1,1,1,1,1,1), "replicate")
        else:
            raise NotImplementedError('padding [%s] is not implemented' % self.padding_type)
